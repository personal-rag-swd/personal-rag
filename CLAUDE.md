...
- `$vectorSearch` uses client-side embeddings: pass `queryVector` (a `list[float]`) on the `embedding` path, not `query`. Embeddings are generated via OpenRouter (`app/core/embedding_provider.py`). Index name is `notebook_chunks_vector_index`. The Atlas index must be defined as `type: vector`, `path: embedding`, `numDimensions: 1536`, `similarity: cosine`, plus `type: filter` fields for every path used in a `$vectorSearch` filter — currently `notebook_id`, `user_id`, and `document_id` (used to scope chat retrieval to a single document). This index lives in Atlas, not in version control, so adding a new filter path in code requires updating the index in every environment first.
