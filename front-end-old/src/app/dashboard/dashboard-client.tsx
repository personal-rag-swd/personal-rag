...
import { createNotebookAction, updateNotebookAction } from "@/features/notebooks/actions"
import {
  type Notebook,
  type NotebookActionState,
} from "@/features/notebooks/types"

const initialNotebookActionState: NotebookActionState = {}
...
function CreateNotebookDialogContent({
  ...
  const [createState, createFormAction, isCreatingNotebook] = useActionState(
    async (_prevState: NotebookActionState, formData: FormData) => {
      const result = await createNotebookAction(initialNotebookActionState, formData)
      if (result.notebook) {
        onSuccess(result.notebook)
        return initialNotebookActionState
      }
      return result
    },
    initialNotebookActionState
  )
  ...
  return (
    <DialogContent className="sm:max-w-md select-none rounded-2xl border-border bg-card">
      ...
      <form action={createFormAction} className="py-2">
        <FieldGroup>
          <Field data-invalid={Boolean(createState.fieldErrors?.name)}>
            ...
            <Input
              ...
              required
            />
            <FieldError>{createState.fieldErrors?.name}</FieldError>
          </Field>

          <Field data-invalid={Boolean(createState.fieldErrors?.description)}>
            ...
            />
            <FieldError>{createState.fieldErrors?.description}</FieldError>
          </Field>

          <Field data-invalid={Boolean(createState.fieldErrors?.tags)}>
            <FieldLabel htmlFor="tags">Tags</FieldLabel>
            <Input
              id="tags"
              name="tags"
              placeholder="e.g. Research, PDF, Technical"
              defaultValue={createState.values?.tags ?? ""}
              aria-invalid={Boolean(createState.fieldErrors?.tags)}
            />
            <FieldDescription>Separate tags with commas.</FieldDescription>
            <FieldError>{createState.fieldErrors?.tags}</FieldError>
          </Field>

          {createState.formError ? <FieldError>{createState.formError}</FieldError> : null}
          ...
          <DialogFooter className="gap-2 sm:gap-0 pt-3">
            ...
            >
              {isCreatingNotebook ? "Creating..." : "Create Workspace"}
            </Button>
          </DialogFooter>
        </FieldGroup>
      </form>
...
function UpdateNotebookDialogContent({
  ...
  const [updateState, updateFormAction, isUpdatingNotebook] = useActionState(
    async (_prevState: NotebookActionState, formData: FormData) => {
      const result = await updateNotebookAction(initialNotebookActionState, formData)
      if (result.notebook) {
        onSuccess(result.notebook)
        return initialNotebookActionState
      }
  ...
  const defaultName = updateState.values?.name ?? notebook.name
  const defaultDescription = updateState.values?.description ?? notebook.description
  const defaultTags = updateState.values?.tags ?? notebook.tags.join(", ")
  ...
  return (
    <DialogContent className="sm:max-w-md select-none rounded-2xl border-border bg-card">
      ...
      <form action={updateFormAction} className="py-2">
        <input type="hidden" name="id" value={notebook.id} />
        <FieldGroup>
          <Field data-invalid={Boolean(updateState.fieldErrors?.name)}>
            <FieldLabel htmlFor="name">Notebook Name</FieldLabel>
            <Input
              id="name"
              name="name"
              ...
              required
            />
            <FieldError>{updateState.fieldErrors?.name}</FieldError>
          </Field>

          <Field data-invalid={Boolean(updateState.fieldErrors?.description)}>
            <FieldLabel htmlFor="description">Description</FieldLabel>
            <Textarea
              id="description"
              name="description"
          ...
          <Field data-invalid={Boolean(updateState.fieldErrors?.tags)}>
            <FieldLabel htmlFor="tags">Tags</FieldLabel>
            <Input
              id="tags"
              name="tags"
              ...
              aria-invalid={Boolean(updateState.fieldErrors?.tags)}
            />
            <FieldDescription>Separate tags with commas.</FieldDescription>
            <FieldError>{updateState.fieldErrors?.tags}</FieldError>
          </Field>

          {updateState.formError ? <FieldError>{updateState.formError}</FieldError> : null}
          ...
          <DialogFooter className="gap-2 sm:gap-0 pt-3">
            ...
            >
              {isUpdatingNotebook ? "Saving..." : "Save Changes"}
            </Button>
          </DialogFooter>
        </FieldGroup>
      </form>
