...
def test_process_minio_notification_payload_skips_unknown_key_safely(session: Session) -> None:
...
def test_process_minio_notification_payload_deduplicates_duplicate_deliveries(session: Session) -> None:
    settings = make_settings()
...
def test_process_minio_notification_payload_retries_transient_failures(session: Session) -> None:
    settings = make_settings()
    document = make_document(session, status="pending")
    payload = {
        "Records": [
...
def test_process_minio_notification_message_reads_real_json_shape(session: Session) -> None:
    payload = {
        "Records": [
            {
                "eventName": "s3:ObjectCreated:Put",
